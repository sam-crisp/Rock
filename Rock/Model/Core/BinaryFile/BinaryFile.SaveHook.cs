                ...
                else
                {
                    ...
                    if ( Entity.MimeType.StartsWith( "image/" ) )
                    {
                        try
                        {
                            ...
                            var binaryFileTypeMaxHeight = binaryFileType.MaxHeight.GetValueOrDefault( 0 );
                            var binaryFileTypeMaxWidth = binaryFileType.MaxWidth.GetValueOrDefault( 0 );
                                ...
                                || ( binaryFileTypeMaxHeightIsValid && binaryFileTypeMaxHeight < Entity.Height ) )
                            {
                                ...
                                {
                                    // Valid max height and width but the max height is greater or equal than the width
                                    if ( binaryFileTypeMaxHeight >= binaryFileTypeMaxWidth )
                                    {
                                        settings.Add( "height", binaryFileTypeMaxHeight.ToString() );
                                    }

                                    // Valid max height and width but the max height is less or equal the width
